...
if (!function_exists('add_scripts')) { // если ф-я уже есть в дочерней теме - нам не надо её определять
	function add_scripts() { // добавление скриптов
	    ...
	    wp_enqueue_script('bcSwipe', get_template_directory_uri().'/js/jquery.bcSwipe.js','','',true); // bcSwipe
	    wp_enqueue_script('backstretch', get_template_directory_uri().'/js/backstretch.min.js','','',true); // backstretch
        wp_enqueue_script('magnific-popup', get_template_directory_uri().'/js/jquery.magnific-popup.js','','',true); // magnific-popup
	    wp_enqueue_script('mCustomScrollbar', get_template_directory_uri().'/js/jquery.mCustomScrollbar.concat.min.js','','',true); // mCustomScrollbar
	    wp_enqueue_script('Owl Carousel', get_template_directory_uri().'/js/owl.carousel.min.js','','',true); // Owl Carousel
        wp_enqueue_script('Mask Input', get_template_directory_uri().'/js/jquery.maskedinput.min.js','','',true); // Mask Input
...
if (!function_exists('add_styles')) { // если ф-я уже есть в дочерней теме - нам не надо её определять
	function add_styles() { // добавление стилей
	    ...
	    wp_enqueue_style( 'bs-theme', get_template_directory_uri().'/css/bootstrap-theme.css' ); // бутстрап-темы
	    wp_enqueue_style( 'input-style', get_template_directory_uri().'/css/input-style.css' ); // input style
	    wp_enqueue_style( 'custom_scroll', get_template_directory_uri().'/css/jquery.mCustomScrollbar.css' ); // custom_scroll
	    wp_enqueue_style( 'magnific-popup', get_template_directory_uri().'/css/magnific-popup.css' ); // magnific-popup
		wp_enqueue_style( 'main', get_template_directory_uri().'/style.css' ); // основные стили шаблона
		wp_enqueue_style( 'fa', '//maxcdn.bootstrapcdn.com/font-awesome/4.6.1/css/font-awesome.min.css' ); // Font Awesome
		wp_enqueue_style( 'Owl Carousel', get_template_directory_uri().'/css/owl.carousel.css' ); // Owl Carousel
		wp_enqueue_style( 'Animate.css', get_template_directory_uri().'/css/animate.css' ); // Animate.css
	}
}

/*----- Моя ф-ция отправки письма ------*/
...
if(!empty($name) && !empty($phone) && !empty($email)) { //Проверка отправилось ли наше поля name и не пустые ли они
        $email_validate = filter_var($email, FILTER_VALIDATE_EMAIL); 
		$to = 'event@jamworkshop.com.ua'; //Почта получателя, через запятую можно указать сколько угодно адресов
        ...
        $headers .= "From: Jam Workshop"; //Наименование и почта отправителя
